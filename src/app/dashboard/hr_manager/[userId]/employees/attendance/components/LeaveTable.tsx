...
import { MoreVertical, ChevronUp, ChevronDown, Eye, Edit, Check, X, Download, FileText } from 'lucide-react';
...
type SortField = 'date' | 'leaveType' | 'employeeName' | 'status';
...
export default function LeaveTable({
  ...
  const getLeaveTypeBadge = (type: string) => {
    ...
    return (
      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${styles[type as keyof typeof styles]}`}>
        {type}
      </span>
    );
  };
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {paginatedRequests.map((request) => (
              <tr key={request.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-400">{request.date}</td>
                <td className="px-4 py-3">{getLeaveTypeBadge(request.leaveType)}</td>
                <td className="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-400">{request.duration}</td>
                <td className="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-400 max-w-xs truncate">{request.reason}</td>
                <td className="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-400">{request.approvedBy || '-'}</td>
                <td className="px-4 py-3">
                  {request.hasDocument ? (
                    <button
